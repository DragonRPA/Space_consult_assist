...
def run_import():
    ...
    grouped.rename(columns={'관측치ID': 'source_count'}, inplace=True)
    ...
    for _, row in grouped.iterrows():
        ...
        params_list.append({
            ...
            "priority": 100,
            "is_active": True,
            "source_engine": "gemma3:12b",
            "action_script": script
        })
    
    with engine.connect() as conn:
        existing = pd.read_sql("SELECT keyword, part_code FROM symptom_rules", conn)
        existing_set = set(zip(existing['keyword'], existing['part_code']))
    
    missing_params = [p for p in params_list if (p['keyword'], p['part_code']) not in existing_set]
    print(f"Skipping {len(params_list) - len(missing_params)} rows. Executing insert for {len(missing_params)} rows...")
    
    chunk_size = 50
    # PUT transaction INSIDE the loop so they commit per chunk!
    for i in range(0, len(missing_params), chunk_size):
        chunk = missing_params[i:i+chunk_size]
        try:
            with engine.begin() as conn:
                sql = text("""
                    ...
                    VALUES 
                    (:keyword, :part_code, :cat1, :cat2, :act_cat, :act_det, :cost, :cnt, :confidence, :priority, :is_active, :source_engine, :action_script)
                    ON CONFLICT (keyword, part_code) DO UPDATE 
    ...
    print("Import and Draft action scripts complete!")
